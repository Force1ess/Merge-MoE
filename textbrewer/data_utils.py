...
def short_disorder(
    tokens, p=[0.9, 0.1, 0, 0, 0]
):  # untouched + four cases abc, bac, cba, cab, bca
    """
...
def long_disorder(tokens, p=0.1, length=20):
    ...
    while i <= len(outputs) - length:
        if np.random.rand() < p:
            outputs[i : i + length // 2], outputs[i + length // 2 : i + length] = (
                outputs[i + length // 2 : i + length],
                outputs[i : i + length // 2],
            )
            i += length
        else:
            i += 1
