...
class LayerNode:
    ...
    def _max_name_length(self, indent1="--", indent2="  ", level=0, max_level=None):
        ...
        if max_level is not None and level >= max_level:
            ...
            child_lengths = [
                child._max_name_length(
                    indent1, indent2, level=level + 1, max_level=max_level
                )
    ...
    for key, value in state_dict.items():
        ...
        for i, name in enumerate(names):
            if name not in current:
                current[name] = LayerNode(
                    name, parent=current, fullname=".".join(names[: i + 1])
                )
        ...
        if (value.data_ptr()) in hash_set:
            ...
            current.value = [
                value.numel(),
                value.numel() * value.element_size() / 1024 / 1024,
            ]
            current.update(current.value)
