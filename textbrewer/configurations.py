...
class IntermediateMatch:
    def __init__(
        self,
        layer_T: Union[int, List[int]],
        layer_S: Union[int, List[int]],
        weight: float,
        loss: str,
        feature: str,
...
class DistillationConfig(Config):
    ...
    def __init__(
        ...
        if hard_label_weight_scheduler != "none":
            assert (
                hard_label_weight_scheduler in WEIGHT_SCHEDULER
            ), "Invalid hard_label_weight_scheduler"
            self.hard_label_weight_scheduler = WEIGHT_SCHEDULER[
                hard_label_weight_scheduler
            ]
        ...
        if kd_loss_weight_scheduler != "none":
            assert (
                kd_loss_weight_scheduler in WEIGHT_SCHEDULER
        ...
        if intermediate_matches:
            self.intermediate_matches = [
                IntermediateMatch.from_dict(im) for im in intermediate_matches
            ]
