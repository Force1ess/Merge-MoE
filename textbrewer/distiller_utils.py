...
class CustomMatch:
    def __init__(
        self, module_T, module_S, weight, loss, proj_func=None, proj_group=None
    ):
...
class DistillationContext:
    ...
    def __enter__(self):
        if isinstance(self.model_T, (list, tuple)):
            ...
            self.model_T_is_training = {
                name: model.training for name, model in self.model_T.items()
            }
        ...
        if isinstance(self.model_S, (list, tuple)):
            ...
            self.model_S_is_training = {
                name: model.training for name, model in self.model_S.items()
            }
    ...
    if len(masks_list) == len(logits_list):
        for logits, mask in zip(logits_list, masks_list):
            if len(logits.shape) == 3:
                ...
                logits_select = torch.masked_select(logits, mask).view(
                    -1, logits.size(-1)
                )
            else:
...
def probability_shift_(
    tensor, labels
):  # In-place operation. shape (batch_size, num_classes), (batch_size,)
...
def get_outputs_from_batch(
    batch, device, model_T, model_S, args, no_teacher_forward=False
):
