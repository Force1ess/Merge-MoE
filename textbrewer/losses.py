...
def att_mse_loss(attention_S, attention_T, mask=None):
    ...
    * If the `inputs_mask` is given, masks the positions where ``input_mask==0``.
    ...
    if mask is None:
        ...
        mask = (
            mask.to(attention_S).unsqueeze(1).expand(-1, attention_S.size(1), -1)
        )  # (bs, num_of_heads, len)
...
def att_ce_loss(attention_S, attention_T, mask=None):
    ...
    * If the `inputs_mask` is given, masks the positions where ``input_mask==0``.
    ...
    if mask is None:
        probs_T_select = torch.where(
            attention_T <= -1e-3, torch.zeros_like(attention_T), probs_T
        )
        loss = -(
        ...
        loss = (
            -(
                (probs_T * F.log_softmax(attention_S, dim=-1) * mask.unsqueeze(2)).sum(
                    dim=-1
                )
                * mask
            ).sum()
...
def att_ce_mean_loss(attention_S, attention_T, mask=None):
    ...
    * If the `inputs_mask` is given, masks the positions where ``input_mask==0``.
    ...
    if mask is None:
        ...
        loss = (
            -(
                (probs_T * F.log_softmax(attention_S, dim=-1) * mask.unsqueeze(1)).sum(
                    dim=-1
                )
                * mask
            ).sum()
...
def cos_loss(state_S, state_T, mask=None):
    ...
    * If the hidden sizes of student and teacher are different, 'proj' option is required in `inetermediate_matches` to match the dimensions.
    ...
    if mask is None:
        ...
        mask = (
            mask.to(state_S).unsqueeze(-1).expand_as(state_S).to(mask_dtype)
        )  # (bs,len,dim)
...
def mmd_loss(state_S, state_T, mask=None):
    ...
    * If the `inputs_mask` is given, masks the positions where ``input_mask==0``.
    ...
    if mask is None:
        gram_S = torch.bmm(state_S_0, state_S_1.transpose(1, 2)) / state_S_1.size(
            2
