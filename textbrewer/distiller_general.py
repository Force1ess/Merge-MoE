...
class GeneralDistiller(BasicDistiller):
    ...
    def __init__(
        ...
        super(GeneralDistiller, self).__init__(
    ...
    def save_and_callback(self, global_step, step, epoch, callback):
        ...
        super(GeneralDistiller, self).save_and_callback(
            global_step, step, epoch, callback
        )
    ...
    def train_on_batch(self, batch, args):
        (teacher_batch, results_T), (student_batch, results_S) = get_outputs_from_batch(
    ...
    def compute_loss(self, results_S, results_T):
        ...
        for ith, inter_match in enumerate(self.d_config.intermediate_matches):
            ...
            losses_dict[f"unweighted_{feature}_{loss_type}_{name_S}_{name_T}"] = (
                intermediate_loss
            )

        if self.has_custom_matches:
            for hook_T, hook_S, match_weight, match_loss, proj_func in zip(
                self.custom_matches_cache["hook_outputs_T"],
                self.custom_matches_cache["hook_outputs_S"],
                self.custom_matches_cache["match_weghts"],
                self.custom_matches_cache["match_losses"],
    ...
    def add_match(self, match: CustomMatch):
        ...
        self.add_match_by_module(
            module_T, module_S, proj_func, proj_group, weight, loss
        )

    def add_match_by_module(
        self,
        module_T: torch.nn.Module,
