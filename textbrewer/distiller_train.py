...
class BasicTrainer:
    ...
    def __init__(self, train_config: TrainingConfig, model: torch.nn.Module, adaptor):
        ...
        **args,
        ...
        if self.t_config.fp16:
            ...
            self.model, optimizer = amp.initialize(
                self.model, optimizer, opt_level=self.t_config.fp16_opt_level
            )
        ...
        if num_steps is not None:
            ...
            for step, batch in tqdm(enumerate(cycle(dataloader)), disable=tqdm_disable):
                ...
                if self.rank == 0:
                    scalar_total_loss = (
                        total_loss.cpu().item()
                        * self.t_config.gradient_accumulation_steps
                    )
                    self.tb_writer.add_scalar(
                        "scalar/total_loss", scalar_total_loss, writer_step
                    )
                writer_step += 1
                ...
                if (step + 1) % self.t_config.gradient_accumulation_steps == 0:
                    ...
                    if (
                        global_step % ckpt_steps == 0
                    ) or global_step == total_global_steps:
                        if self.rank != 0:
                            ...
                            torch.save(
                                state_dict,
                                os.path.join(
                                    self.t_config.output_dir, f"gs{global_step}.pkl"
                                ),
                            )
                ...
                if (step + 1) % self.t_config.gradient_accumulation_steps == 0:
                    ...
                    if (global_step % train_steps_per_epoch in checkpoints) and (
                        ...
                        if self.rank != 0:
                            ...
                            logger.info(
                                f"Saving at global step {global_step}, epoch step {step+1} epoch {current_epoch+1}"
                            )
                            coreModel = (
                                self.model.module
                                if hasattr(self.model, "module")
    ...
    def train_on_batch(self, batch, args) -> torch.Tensor:
        ...
        if "losses" not in results:
            raise KeyError(
                "'losses' not in the output of adaptor. Nothing to optimize!"
            )
        else:
