...
class EVELoraConfig(LoraConfig):
    ...
    expert_num: PositiveInt = field(default=8)
...
class EVELoraModel(LoraModel):
    ...
    def _find_and_replace(self, adapter_name):
        ...
        for key in key_list:
            ...
            #     # 命中不了，是一个list, 命中了attn，但是其实什么也没做'transformer.layers.0.attention.query_key_value'
            ...
            if target_module_found:
                ...
                # 这里也不会命中吧，linear哪里有query_key_value了
                if isinstance(target, EVELoraLayer):
                    target.update_layer(
                        adapter_name,
                        lora_config.init_r,
...
class MMOELoraLinear(nn.Linear, EVELoraLayer):
